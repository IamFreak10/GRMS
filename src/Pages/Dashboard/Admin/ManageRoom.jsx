  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/booking/all-rooms-status?branch=${selectedBranch}`
      );
      ...
      <div className="bg-white p-6 md:p-8 rounded-[35px] shadow-sm border border-base-200 flex flex-col lg:flex-row justify-between items-center gap-6">
        <div>
          <h1 className="text-3xl font-black text-neutral italic">
            {selectedBranch}{' '}
            <span className="text-primary not-italic">Inventory</span>
          </h1>
      ...
      <div className="transition-all duration-500">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-gray-400 font-bold mt-4 animate-pulse uppercase text-[10px] tracking-[0.2em]">
              Syncing {selectedBranch} Data...
            </p>
          </div>
        ) : (
          <>
            ...
            {activeTab === 'add' && (
              <AddRoomForm refreshData={refetch} branch={selectedBranch} />
            )}
          </>
        )}
      </div>
