    ...
    <div className="navbar bg-base-100 shadow-xl px-4 md:px-12 sticky top-0 z-[100]">
      ...
      <div className="navbar-start lg:hidden">
        <div className="dropdown">
          ...
          <ul
            tabIndex={0}
            className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow-2xl bg-base-100 rounded-box w-64 border border-base-200"
          >
            {navLinks.map((link) => (
              <li key={link.path}>
      ...
      <div className="navbar-end">
        {!user ? (
          <div className="flex gap-2">
            ...
            <Link to="/register" className="btn btn-primary btn-sm px-6">
              Join
            </Link>
          </div>
        ) : (
          <div className="flex items-center gap-4">
            <div className="relative group flex items-center justify-center">
              ...
              <div className="absolute -top-10 opacity-0 group-hover:opacity-100 group-hover:-top-14 transition-all duration-500 ease-out pointer-events-none z-50">
                <div className="bg-secondary text-secondary-content text-[10px] uppercase tracking-widest font-bold px-3 py-1.5 rounded-lg shadow-xl">
                  {user?.name} {/* এখানে নাম দেখালে সুন্দর লাগে */}
                </div>
              ...
              <div className="dropdown dropdown-end">
                <label
                  tabIndex={0}
                  className="btn btn-ghost btn-circle avatar online ring-2 ring-primary/20 ring-offset-2 hover:ring-primary transition-all duration-300"
                >
                  <div className="w-10 rounded-full bg-base-200 flex items-center justify-center overflow-hidden">
                    {/* ডাটাবেজের ইমেজ লিংক থাকলে সেটি দেখাবে, না থাকলে আইকন */}
                    {user?.photo_url ? (
                      <img
                        src={user.photo_url}
                        alt="User Profile"
                ...
                <ul
                  tabIndex={0}
                  className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow-2xl bg-base-100 rounded-2xl w-60 border border-base-200"
                >
                  <li className="p-4 border-b border-base-200">
                    <span className="font-bold text-base block leading-none">
                      {user.name}
                    </span>
                    <span className="text-[10px] opacity-60 block mt-1 uppercase tracking-wider">
                      {user.email}
                    </span>
                  </li>
                  <li>
                  ...
                  <li>
                    <Link to="/profile" className="py-3">
                      <FaUserAlt className="text-primary" /> Settings
                    </Link>
                  </li>
